...
Focused on deploying a Windows endpoint and validating end-to-end event collection.

- Wazuh requires both the Manager and Agent to collect endpoint telemetry.
- Windows Event ID 4625 identifies failed authentication attempts.
- Threat Hunting allows analysts to quickly search and investigate endpoint events.
- Proper endpoint enrollment is essential before meaningful monitoring can begin.
- Wazuh maps collected events into searchable security data for investigation.